...
// const { downloadNodes, fixInstitutionInUsers, identifyDuplicateInstitutionDomains } = require("./knowledge");
...
(async () => {
  const functionCaller = process.argv[2];
  //to generate the CSV file for evaluating GPT-4 gradings compared to the grades assigned by every other researcher
  if (functionCaller === "generateTheCSVfileChatGTP") {
    generateTheCSVfileChatGTP();
  }
})();
...
// app.get("/identifyDuplicateInstitutionDomains", identifyDuplicateInstitutionDomains);
...
// app.get("/correctTheDataStructureForDamagedUsers", correctTheDataStructureForDamagedUsers);
// app.get("/deleteDamageDocumentForAffectedUsersInRecallGrades", deleteDamageDocumentForAffectedUsersInRecallGrades);
// app.get("/makeCorrectionToPhrasesinRecallGrades", makeCorrectionToPhrasesinRecallGrades);
