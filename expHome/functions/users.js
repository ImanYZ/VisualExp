...
exports.deleteUser = async (snap, context) => {
  ...
  for (const { passage, condition } of pConditions) {
    try {
...
exports.recalculateFreeRecall = async (req, res) => {
  try {
    ...
    for (const userD of usersDocs.docs) {
      try {
        await db.runTransaction(async t => {
...
exports.reassignAllPConditionNums = async (req, res) => {
  ...
  try {
    ...
    for (const conditionDoc of conditionDocs.docs) {
    ...
    for (const passageDoc of passageDocs.docs) {
    ...
    for (const userDoc of userDocs.docs) {
      ...
      if (userData.pConditions) {
        ...
        for (const { passage, condition } of userData.pConditions) {
...
exports.changeTo3Days = async (req, res) => {
  try {
    const userDocs = await db.collection('users').get();
    for (const userD of userDocs.docs) {
      try {
        await db.runTransaction(async t => {
          const userRef = db.collection('users').doc(userD.id);
          const userDoc = await t.get(userRef);
          ...
          for (const pCondition of newPConditions) {
