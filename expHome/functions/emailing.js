...
exports.loadImageIndividual = (req, res) => {
  const fullname = req.params.contactId.replace('+', ' ');
          ...
          if (!userDoc.exists) {
            ...
            const mailOptions = {
              ...
              html: `<p>Hi ${capitalizeFirstLetter(firstname)},</p>
                ...
                <img src="https://1cademy.us/api/loadImage/individual/${`${getFullnameURI(
                  firstname,
                  lastname,
                )}/${generateUID()}`}"
...
exports.inviteInstructors = async (req, res) => {
  try {
    ...
    for (const instructorDoc of instructorDocs.docs) {
      ...
      if (
        ...
        if (instructorData.condition) {
          ...
          for (const instructorConditionDoc of instructorConditionsDocs) {
            const instructorConditionData = instructorConditionDoc.data();
        ...
        setTimeout(async () => {
          const mailOptions = {
            ...
            )} ${capitalizeFirstLetter(instructorData.lastname)}`},</p>
              <p></p>
...
const hoursToDaysHoursStr = hoursLeft => {
  ...
  if (hoursLeft >= 24) {
    ...
    hoursLeft %= 24;
    return `${days > 1 ? days : 'a'} day${days > 1 ? 's' : ''}${
      hoursLeft > 0
        ? ` and ${hoursLeft > 1 ? hoursLeft : 'an'} hour${
            hoursLeft > 1 ? 's' : ''
          }`
        : ''
    }`;
    ...
    const mailOptions = {
      ...
      subject: `[1Cademy] ${
        ...
        weAreWaiting
          ...
          : declined
          ? `You Have Declined Your ${order} Session of the UX Research Experiment Which Will Begin in ${hoursLeft}!`
      ...
      ${
        weAreWaiting
          ? `<p>Our UX researchers have been waiting for you in the UX research experiment session. ` +
            `Please join the session at <a href=${hangoutLink} target='_blank'>this link</a>.</p><p>` +
          ...
          : declined
          ? `<p>This is an auto-generated email to inform you that you have declined your ${order} session of the UX research experiment which will begin in ${hoursLeft}!</p>` +
          ...
          : `<p>This is an auto-generated email to inform you that your UX research experiment session will begin in ${hoursLeft},` +
...
exports.researcherEventNotificationEmail = async (
  ...
  try {
    ...
    const mailOptions = {
      ...
      ${
        declined
          ? `<p>This is an auto-generated email to inform you that you have declined your ${order} session with ${participant} which will begin in ${hoursLeft}!</p>` +
          ...
          : `<p>This is an auto-generated email to inform you that your UX research experiment session with ${participant} will begin in ${hoursLeft},` +
  ...
  try {
    const mailOptions = {
      ...
      subject:
        `[1Cademy] ` +
        // (courseName ? "[" + courseName + "] " : "") +
        `You Missed the ${order} UX Research Experiment Session Today!`,
      html: `<p>Hi ${capitalizeFirstLetter(firstname)},</p>
        <p></p>
  ...
  try {
    const mailOptions = {
      ...
      html: `<p>Hi ${capitalizeFirstLetter(firstname)},</p>
        ...
        <ul>
        ${applicants.map(applicant => `<li>${applicant}</li>`).join('')}
        </ul>
    ...
    const mailOptions = {
      ...
      html: `<p>Hi Iman,</p>
        ...
        ${needInvite
          .map(
            application =>
              `<li>${application.communiId}, ${application.applicant}</li>`,
          )
          .join('')}
        </ul>
