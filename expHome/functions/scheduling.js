...
const createExperimentEvent = async (email, researcher, order, start, end) => {
  ...
  const description =
    `<div><p><strong><u>IMPORTANT: On your Internet browser, please log in to Gmail using your ${email.toLowerCase()} credentials before entering this session. If you have logged in using your other email addresses, Google Meet will not let you in!</u></strong></p><P>This is your ${order} session of the UX Research experiment.</p>${
      order === '1st'
...
exports.todayPastEvents = async () => {
  try {
    let start = new Date();
    start = new Date(
      `${start.getFullYear()}-${pad2Num(start.getMonth() + 1)}-${pad2Num(
        start.getDate(),
      )}T12:00:00.000Z`,
    );
    let end = new Date();
